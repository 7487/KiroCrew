...
import PromptForm, { assemblePromptContent, parsePromptContent, sanitizePromptName, type PromptFormData, type PromptScope } from '../../components/PromptForm'
...
export default function PromptsTab() {
  ...
  // A write's own error text is the server's, and three codes need translating
  ...
  // compare-and-swap the user never saw; `invalid_name` answers "invalid prompt
  // name" without saying which part of the name was invalid, and a name made
  // entirely of characters outside a-z0-9 is exactly the case that earns it —
  // typically a non-Latin one, so the user least likely to read the English
  // prose is the one most likely to be shown it. The form now catches that
  // client-side too, but the sanitizer there is a mirror rather than the
  // authority, so this stays the honest answer for a name the preview accepted
  ...
  return (<>
    ...
    <Modal open={creating} onClose={() => {
      ...
      {/* Gated on the SANITIZED stem, not the raw name: a name whose every
          character is outside a-z0-9 is non-empty here but empty at the server,
          so gating on `.trim()` sent a request that could only 400. The form's
          hint says why the button is disabled, which is what keeps a disabled
          control from being a dead end. */}
      <Btn primary disabled={!sanitizePromptName(createForm.name) || !createForm.body.trim() || createPrompt.isPending} onClick={() => createPrompt.mutate(createForm)}>{i18nT('pages.overview.promptsTab.create')}</Btn>
    </>}>
