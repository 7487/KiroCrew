...
export default function SidePanelLayout({ title, tabs, defaultTab, rememberKey, footer, headerRight, headerRightDock = 'header', fixedContent, basePath, children }: SidePanelLayoutProps) {
  ...
  // Guards published by the pane currently on screen. A Set rather than one
  // slot because a pane is free to register more than one (a nested editor
  // alongside its host), and a second registration must not silently displace
  // the first. Deleted on the registrant's own unmount, so a stale closure can
  // never hold the rail hostage.
  const leaveGuards = React.useRef(new Set<SidePanelLeaveGuard>())
  const registerLeaveGuard = React.useCallback((guard: SidePanelLeaveGuard) => {
    leaveGuards.current.add(guard)
    return () => { leaveGuards.current.delete(guard) }
  }, [])
  /** Ask the mounted pane before an action that would unmount it. Any single
   *  veto stops the move; a pane with nothing at stake registers no guard and
   *  this is a bare `true`. Guards may show a confirm, so this must only ever
   *  be called from an event handler — never during render. */
  const mayLeavePane = () => {
    for (const guard of leaveGuards.current) if (!guard()) return false
    return true
  }
