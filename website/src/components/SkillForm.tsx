...
function backendFoldsLiteral(block: string, headerEnd: number, header: string): string {
  ...
  /* The +1 is the last line's OWN terminating newline. Every line inside the fence has
     ...
     clip-chomped scalar read one break short of what both parsers give. */
  const trailingBreaks = body.length - end + 1
  const chomp = header.slice(1, 2)
  if (!end) {
    /* An all-blank block's FIRST blank line is the newline that ended the header line,
       not a content break, so keep-chomping preserves one fewer than the break count. */
