...
function readCommand(app: CommandAppRecord, raw: unknown): ContributedCommand | null {
  ...
  // above exist to bound. Clipping is not the "silently shorten, then act on the
  // remainder" this file refuses for the prompt and the argument value: nothing
  // executes a label, and the ellipsis makes the shortening visible where it is read.
  const rawAppLabel = app.displayName || app.name
  let appLabel = rawAppLabel
  if (appLabel.length > MAX_TITLE) {
    appLabel = app.name.length > MAX_TITLE ? `${app.name.slice(0, MAX_TITLE - 1)}\u2026` : app.name
