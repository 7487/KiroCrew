 ...
 *
 * Four frames, because the change has two halves and the second one only shows
 * up in a language other than English:
 *
 *   01-hint-before-typing   the untouched generic rule. This is the sentence the
 *                           field showed for EVERY name before this change --
 *                           `<name>` is a literal -- and now shows only while
 *                           the field is empty.
 ...
 *   04-server-400-japanese  the same refusal arriving from the server, in ja --
 *                           the case Task 1 is about, where the untranslated
 *                           English "invalid prompt name" used to land.
 *
 * Builds nothing: run `npm run build` first if dist is stale, or the "after"
 * frames are indistinguishable from the change not working.
 *
...
try {
  ...
  // ---- 04: the server's own refusal, translated ----------------------------
  ...
  await shoot(ja, '04-server-400-japanese')
  await ja.close()
