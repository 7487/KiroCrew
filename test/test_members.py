...
class TestOverCapRecordFailsClosed:
    ...
    def test_a_cr_delimited_log_does_not_produce_a_duplicate(self):
        """Guards the binary-read conversion itself, not a defect on main.

        These logs were read in TEXT mode, where a bare CR ended a record, so
        `main` parses this file as two records and correctly suppresses the
        repeat. Reading binary splits only on LF, which would glue them into
        one unparseable line -- the prior entry disappears and the probe
        appends a duplicate. The strict reader refuses the record instead, so
        the outcome matches `main`: no duplicate. Red against the first
        revision of this PR (`a97046964`), which appended one.
        """
        log = self._log_for("m")
        ...
        before = log.read_bytes()
        assert record_activity("m", "s1", "persistent", dedupe_session=True) is False
        assert log.read_bytes() == before, "a CR-glued log must not gain a duplicate entry"
