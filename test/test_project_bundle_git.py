...
class TestGitProjectStore:
    ...
    def test_materialize_source_uses_an_existing_checkout_when_fetch_is_offline(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        remote = tmp_path / "remote"
        _committed_bundle(remote)
        ...
        def offline_fetch(cwd: Path, *args: str) -> subprocess.CompletedProcess[str]:
            if args[:1] == ("fetch",):
                raise ProjectGitRefreshUnavailable("Git project operation failed")
            return real_run_git(cwd, *args)

        monkeypatch.setattr(store, "_run_git", offline_fetch)

        resolved = store.materialize_source(
            _PROJECT_ID,
            "api",
            remote.as_uri(),
            on_stale=stale_reasons.append,
        )

        assert resolved == checkout
        assert stale_reasons == ["api"]

    def test_materialize_source_does_not_hide_a_fetch_security_failure(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        remote = tmp_path / "remote"
        _committed_bundle(remote)
        ...
        def refused_fetch(cwd: Path, *args: str) -> subprocess.CompletedProcess[str]:
            if args[:1] == ("fetch",):
                raise ProjectGitError("Git project operation requires an available sandbox")
            return real_run_git(cwd, *args)

        monkeypatch.setattr(store, "_run_git", refused_fetch)

        with pytest.raises(ProjectGitError, match="available sandbox"):
    ...
    def test_materialize_source_reclones_when_the_declared_remote_changes(
        self, tmp_path: Path
    ) -> None:
        first_remote = tmp_path / "first"
        second_remote = tmp_path / "second"
        _committed_bundle(first_remote)
        _committed_bundle(second_remote)
        (second_remote / "new-remote.txt").write_text("second\n", encoding="utf-8")
        _git(second_remote, "add", "new-remote.txt")
        _git(
            second_remote,
            "-c",
            "user.name=Project Tests",
        ...
        registry = ProjectRegistry(tmp_path / "data" / "projects")
        store = GitProjectStore(registry)
        checkout = store.materialize_source(_PROJECT_ID, "api", first_remote.as_uri())

        replaced = store.materialize_source(_PROJECT_ID, "api", second_remote.as_uri())

        assert replaced == checkout
        assert (checkout / "new-remote.txt").read_text(encoding="utf-8") == "second\n"

    def test_materialize_source_force_removes_the_replaced_git_checkout(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        first_remote = tmp_path / "first"
        second_remote = tmp_path / "second"
        ...
        registry = ProjectRegistry(tmp_path / "data" / "projects")
        store = GitProjectStore(registry)
        store.materialize_source(_PROJECT_ID, "api", first_remote.as_uri())
        removed: list[Path] = []
        real_rmtree_force = platform_compat.rmtree_force

        def tracked_rmtree_force(path: str | os.PathLike[str]) -> bool:
            removed.append(Path(path))
            return real_rmtree_force(path)

        monkeypatch.setattr(platform_compat, "rmtree_force", tracked_rmtree_force)

        store.materialize_source(_PROJECT_ID, "api", second_remote.as_uri())

        assert len([path for path in removed if path.name.startswith(".api-replaced-")]) == 1
