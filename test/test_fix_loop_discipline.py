...
class TestHarvestDenominator:
    """A release-branch PR ran that branch's workflow, where the gate is absent.

    Counting those as non-adopters manufactures escapes: the first audit of this
    mechanism reported two 'merged with a green gate and no section' holes, and
    both were release backports the gate had never applied to. The metric has to
    separate 'the gate did not apply' from 'the gate did not run', or every
    weekly report spends its credibility on a phantom.
    """
    ...
    def _fake_gh(prs: list[dict]):
        """Stand in for `gh`: the default-branch probe, then the PR list."""
    ...
    def test_other_base_prs_are_excluded_not_counted_as_escapes(self, monkeypatch) -> None:
        mod = self._module()
        prs = [
            {
                "title": "fix: a",
                "body": "## Pattern harvest\nRule candidate: semgrep",
        ...
        monkeypatch.setattr(mod, "gh_json", self._fake_gh(prs))
    ...
    def test_prs_opened_before_the_gate_are_excluded(self, monkeypatch) -> None:
        """The in-flight queue is not an escape queue.

        18 fix PRs merged in the mechanism's first 35 hours carrying no section
        purely because they were opened against a template that did not have one.
        Reporting those as escapes would have made the metric's first published
        figure almost entirely noise.
        """
        mod = self._module()
        prs = [
            {
                "title": "fix: opened before the gate",
                "body": "no section",
                "mergedAt": "2026-09-01T00:00:00Z",
        ...
        assert out["excluded_pre_gate"] == 1
        assert out["adoption_pct"] is None, "an empty denominator must not render a percentage"

    def test_a_real_escape_is_still_reported(self, monkeypatch) -> None:
        mod = self._module()
        prs = [
            {
                "title": "fix: a",
