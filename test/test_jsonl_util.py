...
class TestBoundedRecordReaders:
    ...
    def test_strict_readers_refuse_a_bare_cr(self, tmp_path):
        """A bare CR ended a record in TEXT mode, so binary reads would glue two.

        The skip reader accepts the narrowing (the glued line just fails to
        parse); a strict caller cannot, because at ``record_activity`` the
        dedupe probe would stop seeing the prior entry and append a duplicate.
        """
        path = self._write(tmp_path, b'{"a":1}\r{"a":2}\n')
        for reader in (strict_records, strict_raw_records):
            with open(path, "rb") as fh:
                with pytest.raises(AmbiguousRecordBoundary):
                    list(reader(fh, path, cap=200))
        # The skip reader still yields the glued line for its caller to reject.
        with open(path, "rb") as fh:
            assert list(bounded_records(fh, path, cap=200)) == ['{"a":1}\r{"a":2}\n']

    def test_strict_readers_allow_a_crlf_terminator(self, tmp_path):
        """That CR rides on the terminator, so it is not an interior boundary."""
        path = self._write(tmp_path, b'{"a":1}\r\n', b'{"a":2}\r\n')
        with open(path, "rb") as fh:
    ...
    def test_every_strict_refusal_is_one_catchable_class(self, tmp_path):
        ...
        assert issubclass(AmbiguousRecordBoundary, UnreadableRecord)
