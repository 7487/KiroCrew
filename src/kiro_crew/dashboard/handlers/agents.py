...
def _promote_pending_avatar(name: str) -> int | None:
    ...
    promoted; None when there was nothing staged. The previously live file
    is kept as a ``.prev`` sibling so a config-save failure can roll the
    promotion back instead of having destroyed the saved picture —
    the caller MUST follow with :func:`_commit_promoted_avatar` (save
    succeeded) or :func:`_rollback_promoted_avatar` (save failed). Runs
    synchronous filesystem work: call via ``asyncio.to_thread``.
    """
