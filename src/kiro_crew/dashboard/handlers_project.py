...
def _capability_payload(status: ProjectCapabilityStatus) -> dict[str, Any]:
    return {
        ...
        "agent_names": [_redact(name) for name in status.agent_names],
        "mcp_server_details": [
            {
                "name": _redact(name),
                **_redact_json_value(spec),
            }
            for name, spec in status.mcp_servers
        ],
    }
...
def _project_payload(
    ...
    if manifest.id != project.id:
        return _unavailable("project_manifest_identity_mismatch")
    ...
    return {
        ...
        # This API is owner-only. Keep display fields redacted, but give the
        # manifest editor the original values so saving another field cannot
        # silently replace credential-shaped prose with a redaction marker.
        "editable": {"name": manifest.name, "description": manifest.description},
