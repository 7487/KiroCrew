...
# Per-TRANSCRIPT metadata-write transaction locks, same shape and rationale as
# chat_handlers._autocompact_txn_lock: with awaits inside a mutate/save/rollback
# span, a second concurrent request would otherwise capture the first one's
# value as its rollback snapshot, and value-based rollback cannot tell "my
# write survived" from "someone else wrote the same value" — so a refused
# request could erase an equal, acknowledged concurrent commit. Under the lock
# exactly one request is inside the span per transcript, so a rollback can only
# undo its own write. Keyed by the transcript so two alias slots resolving onto
# one file serialize too. WeakValueDictionary: an idle transcript's lock is
# reclaimed once no request holds it.
_slot_meta_txn_locks: "weakref.WeakValueDictionary[str, asyncio.Lock]" = (
    weakref.WeakValueDictionary()
)


def _slot_meta_txn_lock(history_key: str) -> asyncio.Lock:
    lock = _slot_meta_txn_locks.get(history_key)
    if lock is None:
        lock = asyncio.Lock()
        _slot_meta_txn_locks[history_key] = lock
    return lock
...
async def api_chat_slot_folder(request: web.Request) -> web.Response:
    ...
    # Serialize the whole re-check/mutate/persist/rollback span per TRANSCRIPT
    # (same rationale as the autocompact endpoint's txn lock): with awaits
    # inside the span, a second concurrent request would capture this one's
    # value as its rollback snapshot, and value-based rollback cannot tell
    # "my write survived" from "someone else wrote the same value". Under the
    # lock a rollback can only undo its own write; the compare-and-set below
    # stays as defense for the non-endpoint writers (the folder-delete unfile
    # loop) that do not take this lock.
    async with _slot_meta_txn_lock(authorized_history_key):
...
async def api_chat_slot_pin(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response({"error": "invalid JSON"}, status=400)
    # Serialize the re-check/mutate/persist/rollback span per TRANSCRIPT —
    # same rationale as api_chat_slot_folder above.
    async with _slot_meta_txn_lock(authorized_history_key):
        # Re-authorize after the awaits above (body parse, lock acquisition):
        # same slot OBJECT still registered under the name, routing still on
        # the transcript captured before the first await. No await between
...
async def api_chat_slot_mode(request: web.Request) -> web.Response:
    ...
    # Serialize the busy-check/re-check/mutate/persist/rollback span per
    # TRANSCRIPT — same rationale as api_chat_slot_folder above. The busy
    # guard runs INSIDE the lock: waiting on a concurrent metadata save can
    # take long enough for a turn to start, so a guard evaluated before the
    # acquisition would be stale by the time the mutation runs (review-caught).
    async with _slot_meta_txn_lock(authorized_history_key):
        # Re-authorize after the awaits above (body parse, lock acquisition):
        # same slot OBJECT still registered under the name, routing still on
