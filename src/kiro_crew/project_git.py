...
class ProjectGitRefreshUnavailable(ProjectGitError):
    """A Git fetch ran but its remote could not be refreshed."""
...
class GitProjectStore:
    ...
    def _run_git(cls, cwd: Path, *args: str) -> subprocess.CompletedProcess[str]:
        ...
        try:
            ...
            error_type = ProjectGitRefreshUnavailable if args[:1] == ("fetch",) else ProjectGitError
            raise error_type("Git project operation timed out") from exc
            ...
            error_type = ProjectGitRefreshUnavailable if args[:1] == ("fetch",) else ProjectGitError
            raise error_type("Git project operation failed") from exc
        finally:
            if cleanup:
                Path(cleanup).unlink(missing_ok=True)
        ...
        with self._lock(f"{project_id}-{source_id}"):
            ...
            if target.exists():
                ...
                if current == remote:
                    ...
                    if not branch:
                        raise ProjectGitError(
                            f"Project repo source has a detached HEAD: {source_id}"
                        )
                    ...
                    except ProjectGitRefreshUnavailable:
                        if on_stale is None:
                            raise
                        on_stale(source_id)
                        return target
    ...
    def remove_project_storage(self, project: RegisteredProject) -> None:
        """Remove only Crew-owned derived state for an unregistered Project."""
        with self._lock(f"{project.id}-storage"):
            state_root = self.registry.projects_dir / "state" / project.id
            ...
            managed_root = self.registry.projects_dir / "managed" / project.id
            managed_bundle = managed_root / "bundle"
            owns_managed_bundle = any(
                registration.origin == "managed_git"
                and registration.path.absolute() == managed_bundle.absolute()
                for registration in project.registrations
            )
            if owns_managed_bundle:
                self._assert_derived_path_unlinked(managed_root)
                if managed_root.exists() and not platform_compat.rmtree_force(managed_root):
                    raise ProjectGitError("managed Project clone could not be removed")
