    ...
    try:
        ...
        try:
            with os.fdopen(fd, "wb") as out, open(src, "rb") as handle:
                ...
                # Before the unlink below, not after: this is the moment the copy
                # stops being a second copy. Buffered bytes live in this process
                # and then in the page cache, so without forcing them out a
                # power-off between the last write and the unlink reaching disk
                # comes back to an empty-or-partial destination and no source —
                # the one outcome a MOVE of the only copy must never produce.
                #
            ...
            with suppress(OSError):
                dst.unlink()
            raise
        # The bytes are durable; the NAME that reaches them still is not, and it is
        # the name the source is about to stop providing.
        #
        # A failing sync takes the destination back out, for the same reason as the
        # staging mover: this function's contract is that it either took the origin or
        # it did not, and a caller told "did not" while the origin now holds a file
        # keeps the manifest entry AND can never restore it — every later attempt sees
        # the origin occupied. Only this call created dst, so removing it is safe.
        try:
            ...
            raise
        src.unlink()
        # best_effort, and nothing raising after the unlink: the restore has already
        # happened here. This caller reports False/raises as "the origin was not
        # taken", which for a file that HAS been put back would strand a restored
        # session outside its own manifest bookkeeping. A non-durable unlink can only
        # resurrect the staged duplicate, never lose the copy now in live storage.
