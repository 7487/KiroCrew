...
def build_spliced_lane(
    ...
    # it rather than assuming both share one. The two lookups resolve to the same
    # block for a lane that still drives both passes from a single step.
    discovery_block = _run_block_with(scalars, "DISCOVERY PASS", contract)
    ...
    pass_block = _run_block_with_instruction(scalars, "FALSIFICATION PASS", contract, stage_dir)
...
def _run_block_with_instruction(
    scalars: list[BlockScalar], needle: str, contract: str, stage_dir: str
) -> str:
    """The `run:` block whose MODEL-FACING instructions carry ``needle``.

    Like ``_run_block_with``, but it matches after ``cat``-spliced prompt files
    are expanded, so an instruction the workflow keeps in a shared prompt file is
    found in the step that splices it rather than only where it appears inline. A
    block whose splices cannot be resolved is skipped, not fatal: an unresolvable
    splice elsewhere in the job is not evidence about the block being looked for.
    """
    for scalar in scalars:
        if scalar.key != "run":
            continue
