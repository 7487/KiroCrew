...
def _url_scan_span(match: re.Match[str]) -> tuple[str, str]:
    ...
    Instead:
      ...
      * AT MOST ONE trailing ``)`` is trimmed, and only when the span's ``)``
        outnumber its ``(`` — so ``[x](…?q)`` loses its single wrapper ``)``,
        ``/wiki/Foo_(bar)`` keeps its balanced one, and a ``…?q=)))…`` run
        (URL content that still travels to the host) stays in the span rather
        than being excluded from the length heuristic;
    ...
    # the single ``)`` that closes ``[x](url)`` — so a balanced ``(bar)`` inside
    # the path is preserved. Never a run: a ``while`` loop here trimmed EVERY
    # unbalanced trailing ``)``, so a query ending in a long pure-``)`` run
    # (legal URL bytes that still travel to the host) was excluded from the
    # span wholesale and the length heuristic saw only the short prefix — an
    # exfil channel. The one stray closer a double-wrapped ``((url))`` leaves
    # behind is merely over-scanned — the safe direction (a bounded false
    # positive, never an unscanned byte).
    if (
        end > 0
