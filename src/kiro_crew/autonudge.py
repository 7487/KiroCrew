...
class AutoNudgeService:
    ...
    async def _monitor_tick_is_quiet(self, loop: NudgeLoop) -> bool:
        ...
        if probe is None:
            return False
        # The target string is the canonical subject ("owner/name#123"), and
        # inference round-trips it into the probe's own config shape. Deriving it
        # here keeps ONE stored spelling of the subject: a second persisted copy
        # of the same fact is a second thing that can disagree with the first.
        target = targets.infer(monitor.target)
        if target is None:
            return False
